...
```
$ go get github.com/stellar/archivist
$ go install github.com/stellar/archivist/cmd/stellar-archivist
```
...
```
NAME:
   Archivist - inspect stellar history archives

USAGE:
   stellar-archivist [global options] command [command options] [arguments...]
...
GLOBAL OPTIONS:
   --low "0"			first ledger to act on
   --high "4294967295"		last ledger to act on
   --last "-1"			number of recent ledgers to act on
   --concurrency, -c "32"	number of files to operate on concurrently
   --s3region "us-east-1"	S3 region to connect to
   --dryrun, -n			describe file-writes, but do not perform any
